...
void Battle::Init() {
    playerTeam = {};
    enemyTeam = {new Slime(SlimeType::Blue), new Slime(SlimeType::Green), new Slime(SlimeType::Yellow)};
    ...
    revivalPotion = 1;   //敌人使用复苏药的数目
}




//玩家选择slime的技能,输入数字，返回相应的技能
...
// 被动死亡下场,游戏结束玩家Lose输出true,游戏继续输出false
...
//如果敌人选择切换slime输出true，选择技能输出false
bool Battle::EnemyChoose(std::ostream& os, Slime* &playSlime0, Slime* &playSlime1, Skill& enemySkill) {
    //敌方当前slime是Blue
    if (playSlime1->getType() == SlimeType::Blue) {
		if (!playSlime1->isAlive()) {   //如果敌方slime已经死亡，选择按顺序切换slime
            if (enemyTeam[1]->isAlive()) {
            ...
            if (weather == Weather::SunnyDay) {
                enemySkill = playSlime1->getSkills()[2];
                return false;
            } else if ((weather != Weather::SunnyDay && playSlime0->getProperty() == Property::Electric) || (weather == Weather::RainyDay && playSlime0->getProperty() == Property::Grass)) {
                if (enemyTeam[1]->isAlive()) {
                    playSlime1 = enemyTeam[1];
                } else if (weather == Weather::RainyDay && enemyTeam[2]->isAlive()) {
                    playSlime1 = enemyTeam[2];
                } else {
                    enemySkill = playSlime1->getSkills()[1];    //Stream
                    return false;
                }
            } else if (weather != Weather::SunnyDay && playSlime0->getProperty() == Property::Water) {
                enemySkill = playSlime1->getSkills()[0];  //Tackle
                return false;
            } else if (weather == Weather::RainyDay) {
                enemySkill = playSlime1->getSkills()[1];  //Stream
                return false;
            } else {
                enemySkill = playSlime1->getSkills()[2];  //Rainy
                return false;
            }
        }
		//敌方当前slime是Green
        ...
        if (!playSlime1->isAlive()) {    //如果敌方slime已经死亡，选择按顺序切换slime
            if (enemyTeam[0]->isAlive()) {
                playSlime1 = enemyTeam[0];
            } else {
                playSlime1 = enemyTeam[2];
            }
        } else {   //敌方slime存活，选择技能
            if (playSlime0->getProperty() == Property::Fire) {
                if (enemyTeam[0]->isAlive()) {
                    playSlime1 = enemyTeam[0];
                } else if (3 * playSlime1->getHP() >= 2 * playSlime1->getMaxHP()) {
                    enemySkill = playSlime1->getSkills()[0];   //Tackle
                    return false;
                } else {
                    enemySkill = playSlime1->getSkills()[2];    //Synthesis
                    return false;
                }
            } else if (playSlime0->getProperty() == Property::Grass) {
                if (2 * playSlime1->getHP() >= playSlime1->getMaxHP()) {
                    enemySkill = playSlime1->getSkills()[0];   //Tackle
                    return false;
                } else {
                    enemySkill = playSlime1->getSkills()[2];    //Synthesis
            ...
            } else if (2 * playSlime1->getHP() >= playSlime1->getMaxHP() || 5 * playSlime0->getHP() < playSlime0->getMaxHP()) {
                enemySkill = playSlime1->getSkills()[1];   //Leaf
                return false;
            } else {
                enemySkill = playSlime1->getSkills()[2];    //Synthesis
                return false;
		...
		//敌方当前slime是Yellow
    } else if (playSlime1->getType() == SlimeType::Yellow) {  
        if (!playSlime1->isAlive()) {    //如果敌方slime已经死亡，选择按顺序切换slime
            if (enemyTeam[0]->isAlive()) {
                playSlime1 = enemyTeam[0];
            } else {
             ...
             if (5 * playSlime0->getHP() < playSlime0->getMaxHP()) {
                enemySkill = playSlime1->getSkills()[0];   //Quick
                return false;
            } else if (playSlime0->getProperty() == Property::Fire) {
                if (enemyTeam[0]->isAlive()) {
                    playSlime1 = enemyTeam[0];
                } else {
                    enemySkill = playSlime1->getSkills()[1];   //Volt
                    return false;
                }
            } else {
                if (enemyTeam[1]->isAlive()) {
                    playSlime1 = enemyTeam[1];
                } else if (playSlime0->getProperty() == Property::Electric || playSlime0->getProperty() == Property::Grass) {
                    enemySkill = playSlime1->getSkills()[0];   //Quick
                    return false;
                } else {
                    enemySkill = playSlime1->getSkills()[1];   //Volt
...
bool Battle::UseRevivalPotion(std::ostream& os) {
    if (revivalPotion > 0) {
        if (!enemyTeam[0]->isAlive()) {
            enemyTeam[0]->setHP(enemyTeam[0]->getMaxHP() / 2);  // 恢复小蓝最大HP的一半
            os << "Enemy uses Revival Potion on " << enemyTeam[0]->getName() << std::endl;
            revivalPotion--;
            return true;
        } else if (!enemyTeam[1]->isAlive() && !enemyTeam[2]->isAlive()) {
            enemyTeam[1]->setHP(enemyTeam[1]->getMaxHP() / 2);  // 恢复小绿最大HP的一半
            os << "Enemy uses Revival Potion on " << enemyTeam[1]->getName() << std::endl;
            revivalPotion--;
            return true;
        }
    }
        ...
        if (!slime1->isAlive()) {
            ...
            if (side) {
                ...
                if (numOfAlive(enemyTeam) > 0) {
                    Skill enemySkill = SkillType::NoSkill;
                    EnemyChoose(os, slime0, slime1, enemySkill);
                } else {
                    os << "You Win" << std::endl;
                    isEnd = true;
                }
...
void Battle::Assist(std::ostream& os, Slime* &slime0, Slime* &slime1, Skill &playSkill, bool side) {
    ...
    if (slime0->isSleeping()) {  //睡着了不可以使用辅助技能
        os << actor << slime0->getName() << " is sleeping!" << std::endl;;
    } else {
        os << actor << slime0->getName() << " uses " << playSkill.getName() << "! ";
        slime0->setSkillUsed();
        if (playSkill.getType() == SkillType::Sing) {
            ...
            int newHP = recoverAmount + slime0->getHP();
            if (newHP > slime0->getMaxHP()) {
                newHP = slime1->getMaxHP();
            }
            slime0->setHP(newHP);
...
//选择back,输出false
bool Battle::Action(std::istream& is, std::ostream& os, Slime* &playSlime0, Slime* &playSlime1, int action, Graph &graph) {
    if (action == 0) {
        os << "You Escape" << std::endl;
        isEnd = true;
        graph.Update(playSlime0, playSlime1, playerTeam, enemyTeam, this->weather);
        return true;
        ...
        Skill enemySkill = playSlime1->getSkills()[0];

        bool enemyAttack = true;
        if (UseRevivalPotion(os)) {
            enemyAttack = false;
        }else if (EnemyChoose(os, playSlime0, playSlime1, enemySkill)) {
            enemyAttack = false;
        }

        graph.Update(playSlime0, playSlime1, playerTeam, enemyTeam, this->weather);  //Draw
...
//一个回合的操作
void Battle::OneRound(std::istream& is, std::ostream& os, Slime* &playSlime0, Slime* &playSlime1, Graph& graph) {
    graph.Update(playSlime0, playSlime1, playerTeam, enemyTeam, this->weather);
    os << "--------------------------------------------------" << std::endl;
    // 回合开始前，处理天气和睡眠状态
    ...
    if (numOfAlive(playerTeam) > 1) {
        while (true) {
            ...
            is >> action;
            if ((action == 0 || action == 1 || action == 2) && Action(is, os, playSlime0, playSlime1, action, graph)) {
                break;
            }
        }
        ...
        while (true) {
            ...
            is >> action;
            if ((action == 0 || action == 1) && Action(is, os, playSlime0, playSlime1, action, graph)) {
                break;
            }
        }
    }
...
void Battle::play(std::istream& is, std::ostream&os) {
    ...
    if (easy2d::Game::init()) {
        ...
        Graph graph(playerTeam, enemyTeam);

        graph.Update(playSlime0, playSlime1, playerTeam, enemyTeam, this->weather);  //Draw

        //开始游戏
        while (!isEnd) {
            OneRound(is, os, playSlime0, playSlime1, graph);
            round++;
            if (round == 101) {
                os << "Draw" << std::endl;
                isEnd = true;
